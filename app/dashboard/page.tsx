...
function detailedProgress(status: RunStatus | null | undefined): number {
  if(!status) return 0;
    ...
    <div className="grid" style={{gap:'32px'}}>
      ...
      <div className="grid" style={{gap:24, gridTemplateColumns:'repeat(auto-fit,minmax(320px,1fr))'}}>
        ...
        <div className="card" style={{display:'flex', flexDirection:'column', gap:12}}>
          ...
          {status?.errors && status.errors.length > 0 && <div className="error-text" style={{fontSize:'.6rem'}}>{status.errors.join(', ')}</div>}
        </div>
      </div>
